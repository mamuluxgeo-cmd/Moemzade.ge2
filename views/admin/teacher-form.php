...
<section class="section admin-section"><div class="container"><form class="admin-panel teacher-form" action="/admin/teachers/save" method="post" enctype="multipart/form-data" data-taxonomy-form data-location-form>
    ...
    <fieldset><legend>კატალოგი</legend>
        ...
        <label><span><?= e(t('search.category')) ?></span><select name="category" data-category-select required><option value=""><?= e(t('search.any')) ?></option><?php if (!empty($teacher['category']) && !in_array((string) $teacher['category'], $options['categories'], true)): ?><option value="<?= e($teacher['category']) ?>" selected><?= e($teacher['category']) ?></option><?php endif; ?><?php foreach ($options['categories'] as $item): ?><option value="<?= e($item) ?>" <?= ($teacher['category'] ?? '') === $item ? 'selected' : '' ?>><?= e($item) ?></option><?php endforeach; ?></select></label>
