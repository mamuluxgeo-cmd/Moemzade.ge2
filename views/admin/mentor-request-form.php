...
<section class="section admin-section"><div class="container"><form class="admin-panel teacher-form" action="/admin/mentor-requests/save" method="post" data-taxonomy-form data-location-form>
    ...
    <fieldset><legend><?= e(t('mentor.step1_title')) ?></legend>
        ...
        <label><span><?= e(t('search.category')) ?></span><select name="category" data-category-select required><option value="">—</option><?php foreach ($options['categories'] as $item): ?><option value="<?= e($item) ?>" <?= $value('category') === $item ? 'selected' : '' ?>><?= e($item) ?></option><?php endforeach; ?></select></label>
