...
<section class="section listing-section">
    <div class="container">
        <form class="filter-form" action="/teachers" method="get">
            ...
            <label><span><?= e(t('search.region')) ?></span><select name="region"><option value=""><?= e(t('search.any')) ?></option><?php foreach ($options['regions'] as $item): ?><option value="<?= e($item) ?>" <?= $filters['region'] === $item ? 'selected' : '' ?>><?= e($item) ?></option><?php endforeach; ?></select></label>
        ...
        <?php if ($teachers): ?>
            <div class="teacher-grid"><?php foreach ($teachers as $teacher): require BASE_PATH . '/views/partials/teacher-card.php'; endforeach; ?></div>
        <?php else: ?>
